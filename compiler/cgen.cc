...
void program_class::cgen(ostream& os) {
  ...
  auto codegen_classtable = new CgenClassTable(classes, os);
...
void CgenClassTable::setup_class_tags() {
  int index = 0;
  ...
  nds.sort([](const CgenNodeP& a, const CgenNodeP& b) {
...
int CgenClassTable::setup_class_tags_helper(CgenNodeP root, int& index) {
  auto classtag = index++;
...
void CgenClassTable::code_proto_object() {
  int idx = 0;
  for (auto cls: nds) {
    stringstream ss;
