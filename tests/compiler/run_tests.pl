...
if ( @ARGV lt 2 ) {
    ...
    print STDERR "\t-o\t\toverwrite file w/o prompt\n";
    print STDERR "\t-f filename\tsave to filename instead\n\n";
...
$executable="coolc";
if ($testcase eq "lexer") {
  $executable="coolc -L";
}
if ($testcase eq "parser") {
  $executable="coolc -P";
}
if ($testcase eq "semant") {
  $executable="coolc -S";
}
...
print "Testing binary in $binarydir\n";
...
# the will contain all of the script generated
...
foreach my $caseref (@cases) {
	...
	if ( $testcase eq "lexer" ) {
	    ...
	    $retval = system("$binarydir/$executable $TESTS_DIR/$casefile > $testout 2>&1");
	} elsif ( $testcase eq "parser" ) {
	    $retval = system("$binarydir/$executable $TESTS_DIR/$casefile > $testout 2>&1");
	    $retval = 0; #if there were errors, the parser returns an
	    # error value
	} elsif ( $testcase eq "semant" ) {
	    $retval = system("$binarydir/$executable $TESTS_DIR/$casefile > $testout 2>&1");
	    $retval = 0;
	} elsif ( $testcase eq "cgen" ) {
		...
		$retval = system("$binarydir/$executable $addedflags -g -o $casefile.s $TESTS_DIR/$casefile");
	    } else {
		$retval = system("$binarydir/$executable $addedflags -o $casefile.s $TESTS_DIR/$casefile");
	    }
	    if (($retval != 0)) {
		system("mv $casefile.s $testout");
	    } else {
		$retval = system("spim -trap_file ../../runtime/mips.handler -file $casefile.s > $testout 2>&1");
		system("rm $casefile.s");
	    }
	} elsif ( $testcase eq "optimize" ) {
	    #send output to /dev/null because some people have _insane_ amounts
	    #of debug output.
	    $retval = system("$binarydir/$executable $addedflags -g -O -o $casefile.s $TESTS_DIR/$casefile > /dev/null 2>&1 ");
	    if (($retval != 0)) {
		system("mv $casefile.s $testout");
	    } else {
...
#    close TEST;
#    print "appending COMMENTS.ta to file\n";
